...
RenderTexture2D LoadStrippedRenderTexture(int width, int height)
{
	...
	if (target.id > 0) {
		...
		if (rlFramebufferComplete(target.id)) {
			TraceLog(LOG_INFO, "FBO: [ID %i] CUSTOM Framebuffer object created successfully", target.id);
		}
		rlDisableFramebuffer();
	} else {
		TraceLog(LOG_WARNING, "FBO: CUSTOM Framebuffer object can not be created");
	}
	return target;
}

void DrawTextureCentered(Texture2D t, float x, float y, float xscale, float yscale, float rotation, Color tint)
{
